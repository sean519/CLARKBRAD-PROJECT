const canvas = document.querySelector("#field");
    ...
    const doorReadout = document.querySelector("#doorReadout");
    const doorTrack = document.querySelector("#doorTrack");
    ...
    function awardUfoUpgrade() {
      ...
      game.lastUpgrade = `Rescue reward: +1 tech point. Spend it in the UFO Tech Tree.`;
    ...
    function grantEnergy(amount, reason, x, y, t = performance.now()) {
      ...
      while (game.evolutionTier < evolutionNames.length - 1 && game.energy >= energyNeededForEvolution()) {
    ...
    function renderTechTree() {
      ...
      const html = techCatalog.map((tech) => {
        ...
        const disabled = points <= 0 || maxed || locked;
        const body = locked ? techRequirementText(tech) : tech.body;
        return (
          `<button class="tech-node ${maxed ? "is-maxed" : ""} ${locked ? "is-locked" : ""}" type="button" data-tech="${tech.type}" ${disabled ? "disabled" : ""}>` +
    ...
    function spendTechPoint(type) {
      ...
      if (!tech || (game.skillPoints || 0) <= 0) return;
      if (isTechLocked(tech)) return;
      const level = game.upgrades[type] || 0;
      if (level >= tech.max) return;
      game.skillPoints -= 1;
      game.upgrades[type] = level + 1;
      applyTechUpgrade(type);
      game.lastUpgrade = `Tech upgraded: ${tech.title} ${game.upgrades[type]}/${tech.max}.`;
    ...
    function evolveUfo(reason, t = performance.now()) {
      ...
      game.lastUpgrade = `EVOLUTION Lv.${tier + 1}: ${evolutionNames[tier]}. ${reward.points ? `+${reward.points} tech point${reward.points > 1 ? "s" : ""}.` : reward.text}`;
    ...
    function makeNodes() {
      ...
      const count = availableElements.length;
      nodes = Array.from({ length: count }, (_, index) => {
        const element = availableElements[index % availableElements.length];
    ...
    function growBlackHoleFromMeals() {
      const meals = Math.min(14, blackHole.eatenCount + blackHole.swallowed.length * 0.45);
      const targetRadius = blackHole.baseRadius + meals * 4.2;
      const targetInfluence = blackHole.baseInfluence + meals * 14;
    ...
    function drawSpaceStation(t) {
      ...
      ctx.shadowBlur = dockReady ? 30 : 18;
    ...
    function drawElementTile(node, t) {
      ...
      ctx.shadowBlur = isDanger || isCargo || isHeld || isTrapped || node.selected || isHovered || (activeFamily && isFamilyMatch) ? 24 : 8;
      ctx.shadowOffsetY = isCargo || isHeld || isTrapped || node.selected || isHovered ? 0 : 4;
    ...
    function addWaterEffect(x, y, now) {
      effects.push({ type: "water", x, y, born: now, drops: Array.from({ length: 34 }, () => ({
        angle: randomBetween(0, Math.PI * 2),
    ...
    function addExplosionEffect(x, y, now) {
      effects.push({ type: "explosion", x, y, born: now, particles: Array.from({ length: 76 }, () => ({
        angle: randomBetween(0, Math.PI * 2),
